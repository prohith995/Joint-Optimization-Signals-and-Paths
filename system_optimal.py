...
f = open("Converging_SO.txt" , "w+")
g = open("Link_Flows_SO_SS.txt" , "w+")
...
def margtime(j):
	time = t0[j] * (1 + 0.75*math.pow(flow[0][j]*1.0/cap[j],4))
	return time
...
def sp(i,j): 
	...
	# Step 3 : Arc Processing
		for l in range( point[node] , point[node+1] ):
			...
			time[l] = margtime(l) + mardelay( head[l] , tail[l] , flow[0][l] )
